  ...
  return (
    <main className="grid min-h-screen bg-slate-950 p-4 text-white lg:grid-cols-[1fr_480px] lg:p-8">
      <section className="hidden flex-col justify-between rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,#2563eb,transparent_32%),linear-gradient(135deg,#0f172a,#111827)] p-10 lg:flex">
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.35em] text-blue-200">Notes MVP</p>
          <h1 className="mt-6 max-w-2xl text-6xl font-black leading-none tracking-tight">Notas privadas, rapidas y listas para pantalla compartida.</h1>
        </div>
        <p className="max-w-lg text-lg leading-8 text-slate-300">Organiza grupos, prioridades y fechas limite con una interfaz tipo escritorio que tambien funciona en movil.</p>
      </section>
      ...
      <section className="mx-auto flex w-full max-w-md items-center">
        <form onSubmit={handleSubmit} className="w-full rounded-[2rem] border border-white/10 bg-white p-6 text-slate-950 shadow-2xl md:p-8">
          <div className="mb-8">
            ...
            <h2 className="mt-2 text-3xl font-black tracking-tight">{mode === 'login' ? 'Iniciar sesion' : 'Crear cuenta'}</h2>
